#!/bin/bash
# Script to extract PostGIS Raster extension views and functions across PostgreSQL versions

# Save the current directory
SCRIPT_DIR=$(pwd)

# Create directory for output files
mkdir -p postgis_raster_extracts

# Create the SQL file for PostGIS raster analysis
cat > postgis_raster_extractor.sql << 'EOF'
...
echo "Created SQL extraction script for PostGIS Raster"

# Define PostgreSQL versions to test (using PostGIS Docker images)
PG_VERSIONS=("13" "14" "15" "16" "17")

# Extract PostGIS Raster schemas for each PostgreSQL version
for pg_version in "${PG_VERSIONS[@]}"; do
    echo ""
    echo "========================================================================"
    echo "Extracting PostGIS Raster for PostgreSQL $pg_version"
    echo "========================================================================"
    
    CONTAINER_NAME="postgis_raster_extract_pg${pg_version}"
    ...
    # Run the raster extraction script
    echo "Running raster extraction script for PostgreSQL $pg_version..."
    docker cp postgis_raster_extractor.sql $CONTAINER_NAME:/tmp/
    docker exec $CONTAINER_NAME bash -c "psql -U postgres -f /tmp/postgis_raster_extractor.sql" > postgis_raster_extracts/raster_schema_pg${pg_version}.out 2>&1
    
    # Stop and remove container
    docker stop $CONTAINER_NAME >/dev/null 2>&1
    docker rm $CONTAINER_NAME >/dev/null 2>&1
    
    echo "Raster schema extracted to postgis_raster_extracts/raster_schema_pg${pg_version}.out"
done

# Generate diffs between consecutive PostgreSQL versions
echo ""
echo "========================================================================"
echo "Generating diffs between consecutive versions for Raster"
echo "========================================================================"
for i in $(seq 0 $((${#PG_VERSIONS[@]}-2))); do
    current=${PG_VERSIONS[$i]}
    next=${PG_VERSIONS[$((i+1))]}
    echo "Comparing PostgreSQL $current vs $next PostGIS Raster schemas..."
    diff -u postgis_raster_extracts/raster_schema_pg${current}.out postgis_raster_extracts/raster_schema_pg${next}.out > postgis_raster_extracts/raster_pg${current}_to_pg${next}_diff.patch
done

echo ""
echo "PostGIS Raster analysis complete!"
echo "Files generated:"
ls -la postgis_raster_extracts/

# Provide a summary of findings
echo ""
echo "=== Summary of Extracted Information ==="
echo "For each PostgreSQL version, extracted:"
echo "- Raster extension version and installation status"
echo "- Raster views (raster_columns, raster_overviews)"
echo "- Raster-related functions (ST_* raster functions)"
echo "- Raster data types (raster, rastbandarg, etc.)"
echo "- View definitions for raster views"
echo "- System catalog information"
echo "- Sample raster data (if any raster tables exist)"

echo ""
echo "=== Analysis Commands ==="
echo "View raster extraction for specific version:"
echo "  cat postgis_raster_extracts/raster_schema_pg17.out"
echo ""
echo "Check differences between versions:"
echo "  cat postgis_raster_extracts/raster_pg16_to_pg17_diff.patch"
echo ""
echo "Search for specific raster elements:"
echo "  grep -A 5 'RASTER VIEW COLUMN' postgis_raster_extracts/raster_schema_pg*.out"
echo "  grep -A 10 'RASTER FUNCTION' postgis_raster_extracts/raster_schema_pg*.out"
echo "  grep -A 5 'RASTER DATA TYPES' postgis_raster_extracts/raster_schema_pg*.out"

echo ""
echo "Key raster elements to look for:"
echo "- raster_columns (main raster catalog view)"
echo "- raster_overviews (raster overview relationships)"
echo "- raster data type"
echo "- ST_AddRasterConstraints(), ST_DropRasterConstraints() functions"
echo "- ST_Band(), ST_NumBands(), ST_Width(), ST_Height() functions"
echo "- ST_MakeEmptyRaster(), ST_AddBand() functions"

echo ""
echo "Clean up:"
echo "rm -rf postgis_raster_extracts/ postgis_raster_extractor.sql"
echo ""
echo "Done!"